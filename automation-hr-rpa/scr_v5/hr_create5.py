...
from sendmail2 import automail_send_status2
from your_password_module import load_password  # import ฟังก์ชันจากไฟล์ที่บันทึกรหัสไว้
from screen_log import capture_screen
from log_function import write_log
...
# statuslog_path = os.path.join(current_script_path, template_config, "logs")
#print(statuslog_path)

# หาไฟล์ .csv ล่าสุด ใน folder ของ hr
excel_files = [f for f in os.listdir(hr_folder) if f.endswith('.csv')]
if not excel_files:
    #print("ไม่พบไฟล์ .csv ในโฟลเดอร์:", hr_folder)
    sys.exit(1)  # หยุดโปรแกรมทันทีด้วย exit code 1 (แสดงว่า error)

full_paths = [os.path.join(hr_folder, f) for f in excel_files] # สร้าง path เต็ม
latest_file = max(full_paths, key=os.path.getmtime) # หาไฟล์ที่ใหม่ที่สุด (ตามเวลาแก้ไขล่าสุด)
hr_filename = os.path.basename(latest_file) # แสดงชื่อไฟล์ (เอาเฉพาะชื่อ ไม่เอา path)
#print(f"ใช้ไฟล์ Excel ล่าสุด: {hr_filename}")

# อ่านข้อมูลจากไฟล์ของ hr .csv
time.sleep(3)
input_hrpath = os.path.join(hr_folder, hr_filename)
# df_create = pd.read_csv(input_hrpath, dtype=str, encoding="utf-8-sig")
# print(df_create)

# สร้างชื่อ folder ตาม format ddmmyyyy_hhmmss
# folder data ใช้ภายในโปรแกรมนี้เท่านั้น เพื่อเก็บไฟล์ที่เกี่ยวข้องกับการสร้าง user เช่น log และไฟล์ hr ที่ copy มา
folder_data_name = datetime.now().strftime("data_%d%m%Y_%H%M%S")
folder_data_path = os.path.join(current_script_path,'data', folder_data_name)
folder_data_log = os.path.join(folder_data_path, 'logs')
folder_data_log_error = os.path.join(folder_data_path, 'logs', 'log_error')
# สร้าง folder
os.makedirs(folder_data_path, exist_ok=True)
os.makedirs(folder_data_log, exist_ok=True)
os.makedirs(folder_data_log_error, exist_ok=True)

# copy file hr ไป folder data
shutil.copy2(input_hrpath, folder_data_path)
df_create = pd.read_csv(input_hrpath, dtype=str, encoding="utf-8-sig")
#print(df_create)
...
try:
    write_log(folder_data_log, "INFO", "Start processing", "sap_system")
    ...
    for index, row in df_config_login.iterrows():
        ...
        #role_list
        if sap_system in ['S4D','S4P' , 'S/4 HANA - PRD', 'S4Q', 'CU S4D', 'CU S4Q'] :
    ...
    #    
        ...
        folder_data_log2 = os.path.join(folder_data_log, f"status_log_{safe_sap_system}_{start_script_time}.xlsx")
        df_log2.to_excel(folder_data_log2, index=False)
        ...
        "total_users": len(df_create),
        "log_path": folder_data_log2,
        "data_path": os.path.join(folder_data_path, hr_filename)
        }
        all_mail_results.append(mail_info)
        
    #automail_send_status(all_mail_results, hr_filename, input_mailto, latest_file)
    automail_send_status2(all_mail_results,input_mailto)
    ...
    # path ปลายทางแบบเต็ม
    done_path = os.path.join(hr_done, hrfile_name)
    # ย้ายไฟล์
    shutil.move(input_hrpath, done_path)
    #print(f"ย้ายไฟล์ไปที่: {done_path}")
    ...
    # ใช้ sap_system จริง (กัน undefined)
    current_system = sap_system if 'sap_system' in locals() else "UNKNOWN"

    # --- 1. CAPTURE SCREEN ---
    try:
        screenshot_path = capture_screen(folder_data_log_error, current_system)
    except Exception as screen_err:
        screenshot_path = None
        print("Screenshot failed:", screen_err)
...
{screenshot_path}
"""
        write_log(folder_data_log_error, "ERROR", full_error_log, current_system)

    except Exception as log_err:
        print("Write log failed:", log_err)
    ...
    mail_error = [
        {
            "sap_system": current_system,
            "total_users": "ERROR",
            "log_path": os.path.join(folder_data_log_error, "error.png"),
            "data_path": os.path.join(folder_data_path, hr_filename)
        }
    ]

    try:
        automail_send_status2(mail_error, input_mailto)
    except Exception as mail_err:
        print("Send mail failed:", mail_err)
